...
class AnimateFromCoeff():
    ...
    def convert_video_to_h264(self, input_path, output_path):
        command = [
            'ffmpeg',
            '-i', input_path,
            '-vcodec', 'libx264',
    ...
    def save_video_with_ffmpeg(self, frames, audio_path, output_path, fps=25):
        # First, save frames as images
        temp_frame_dir = os.path.join(os.path.dirname(output_path), 'temp_frames')
        os.makedirs(temp_frame_dir, exist_ok=True)
        
        for i, frame in enumerate(frames):
            cv2.imwrite(os.path.join(temp_frame_dir, f'frame_{i:04d}.png'), cv2.cvtColor(frame, cv2.COLOR_RGB2BGR))
        ...
        ffmpeg_command = [
            ...
            '-i', audio_path,
            '-c:v', 'libx264',
            '-preset', 'medium',  # Adjust preset as needed (e.g., 'slow' for better compression, 'fast' for quicker encoding)
            '-crf', '23',  # Adjust CRF value as needed (lower for better quality, higher for smaller file size)
            '-c:a', 'aac',
            '-b:a', '192k',
            '-pix_fmt', 'yuv420p',  # Ensure pixel format compatibility
    ...
    def generate(self, x, video_save_dir, pic_path, crop_info, enhancer=None, background_enhancer=None, preprocess='crop', img_size=256):
        ...
        # Use the new FFmpeg-based method to save the video
        self.save_video_with_ffmpeg(result, audio_path, path)
        ...
        if enhancer:
            ...
            try:
                ...
                self.save_video_with_ffmpeg(enhanced_images_gen_with_len, audio_path, enhanced_path)
            except:
                enhanced_images_gen_with_len = enhancer_list(path, method=enhancer, bg_upsampler=background_enhancer)
                self.save_video_with_ffmpeg(enhanced_images_gen_with_len, audio_path, enhanced_path)
            
            os.replace(enhanced_path, av_path_enhancer)
            print(f'The generated video is named {av_path_enhancer}')
            os.remove(path)
            path = av_path_enhancer

        # Handle full image processing if required
        if 'full' in preprocess.lower():
            video_name_full = x['video_name'] + '_full.mp4'
            full_video_path = os.path.join(video_save_dir, video_name_full)
            return_path = full_video_path
            paste_pic(path, pic_path, crop_info, audio_path, full_video_path, extended_crop=('ext' in preprocess.lower()))
            print(f'The generated video is named {full_video_path}')
            os.remove(path)
        else:
            os.replace(path, av_path)
